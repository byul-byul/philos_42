...
/*   Updated: 2025/03/05 14:48:13 by bhajili          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static void	init_philo(t_philo *philo, int order, t_fork *left, t_fork *right)
{
	philo->id = order;
	philo->left = left;
	philo->right = right;
...
static void	init_philo_list(t_data *d)
{
	...
	&d->fork_list[n - 1], &d->fork_list[i]);
	if (n > 1)
		init_philo(&d->philo_list[n - 1], n, \
		&d->fork_list[n - 2], &d->fork_list[n - 1]);
	else
		init_philo(&d->philo_list[n - 1], n, \
		&d->fork_list[n - 1], &d->fork_list[n - 1]);
	while (++i < n - 1)
		init_philo(&d->philo_list[i], i + 1, \
		&d->fork_list[i - 1], &d->fork_list[i]);
}
